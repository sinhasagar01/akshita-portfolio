...
- [ ] Semantic markup and real alt text on every image. Still NOT done, verified. fosfor-data-profiling and elevate-one-view are 100 percent placeholder alt, fosfor-ai is part real (6 of 13). Clears as real screens are uploaded with real alt through /studio. The About portrait alt is real ("Portrait of Akshita").
...
- [x] Contact form endpoint set. The owner set NEXT_PUBLIC_CONTACT_FORM_ENDPOINT and the Web3Forms key in Vercel. Could not independently confirm the inlined value from outside (it is a client bundle var in a webpack runtime-loaded chunk, and the value only inlines on a rebuild after the env change), so the definitive check is a one-time form submission. Marked done on the owner ops confirmation.
