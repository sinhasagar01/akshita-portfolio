...
export default function StudioSidebar() {
  ...
  const areas: Area[] = [
    ...
    { href: "/studio/gallery", label: "Gallery", Icon: IconImage, count: counts.gallery },
    ...
    return (
        ...
        className={[
          ...
          active
            ? // THE INK PILL IS INVERTED AT `lg`, AND IT HAD TO BE. `bg-ink-950` on the ink
              ...
              "bg-studio-ink-950 lg:bg-white/10 font-medium"
      ...
      >
        {/* THE ink-400 COLOUR UTILITY IS KEPT AT BOTH BREAKPOINTS, AND THAT IS A CHECKED DECISION RATHER
            ...
            re-check it. */}
        <area.Icon className={`size-4 ${active ? "text-studio-cream-50" : "text-studio-ink-400"}`} />
        ...
        <span
          className={
            active
              ? "text-studio-cream-50"
              : "text-studio-ink-600 group-hover:text-studio-ink-950 lg:text-studio-ink-200 lg:group-hover:text-studio-cream-50"
          }
        >
          {area.label}
        </span>
        {area.count != null && (
          ...
          <span
            className={`ml-auto text-[12px] ${active ? "text-studio-cream-50/70" : "text-studio-ink-600 lg:text-studio-ink-400"}`}
          >
            {area.count}
          </span>
        )}
  ...
  return (
    ...
    <aside id="studio-sidebar" className="flex flex-col border-b border-studio-ink-950/12 bg-studio-cream-100 p-3 lg:sticky lg:top-0 lg:h-screen lg:w-[var(--studio-sidebar-w)] lg:flex-none lg:overflow-y-auto lg:border-b-0 lg:border-r lg:border-white/24 lg:bg-studio-ink-950 lg:p-4">
      <div className="flex items-center gap-2.5 px-1.5 pb-3 lg:pb-4">
        {/* 30x30 AND 20px ARE THE CONTRACT'S OWN VALUES, verified in the file rather than
            ...
            fill did not move. */}
        <span className="grid size-[30px] place-items-center rounded-[var(--studio-radius-card,8px)] bg-studio-accent-500 text-studio-cream-50">
        ...
        <span className="font-display text-[20px] tracking-[-0.01em] text-studio-ink-950 lg:text-studio-cream-50">Studio</span>
      </div>
      ...
      <nav className="flex flex-1 flex-row gap-1 overflow-x-auto lg:flex-col lg:gap-0.5 lg:overflow-visible">
        ...
        <div className="lg:mt-auto lg:border-t lg:border-white/12 lg:pt-2.5">
          ...
          <form action="/api/studio/logout" method="post">
            ...
            <button
              type="submit"
              className="flex w-full items-center gap-2.5 whitespace-nowrap rounded-[var(--studio-radius-control,4px)] px-2.5 py-2 text-[14px] text-studio-ink-600 transition-colors hover:bg-studio-cream-50/70 hover:text-studio-ink-950 lg:text-studio-ink-200 lg:hover:bg-white/5 lg:hover:text-studio-cream-50"
            >
              <IconLogout className="size-4 text-studio-ink-400" />
              <span>Sign out</span>
            </button>
          </form>
        </div>
