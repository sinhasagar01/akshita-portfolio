...
function DiffLine({ line }: { line: ChangedLine }) {
  ...
  return (
    <div className="flex gap-2 py-px">
      <span
        aria-hidden="true"
        className={`shrink-0 select-none font-medium ${added ? "text-success-700" : "text-danger-600"}`}
      >
        {added ? "+" : "−"}
      </span>
      <span className="min-w-0 break-words text-ink-800">{line.text || " "}</span>
  ...
  return (
    ...
    >
      <div id="publish-preview-body" className="mt-4 min-h-0 flex-1 overflow-y-auto">
        {state.kind === "loading" && (
          <p className="text-[14px] text-text-subtle">Loading what will change…</p>
        )}
        ...
        {state.kind === "unavailable" && (
          ...
          <p className="text-[14px] text-ink-800">{state.reason}</p>
        )}
        ...
        {ready && ready.fileCount > 0 && (
          <>
            <p className="text-[14px] text-ink-950">
            ...
            {ready.truncated && (
              ...
              <p className="mt-2 rounded-[var(--studio-radius-control,4px)] border border-ink-950/12 bg-cream-200 px-3 py-2 text-[13px] text-ink-800">
            ...
            <ul className="mt-4 flex flex-col gap-3">
              {ready.entries.map((entry, i) => (
                <li
                  ...
                  className="rounded-[var(--studio-radius-control,4px)] border border-ink-950/12 bg-cream-50 p-3"
                >
                  <p className="text-[14px] font-medium text-ink-950">{entry.title}</p>
                  <p className="mt-px text-[12.5px] text-text-subtle">
