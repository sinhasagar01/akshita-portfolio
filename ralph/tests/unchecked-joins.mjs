...
const DECLARED = {
  ...
  "lib/studio/search-index.ts": "`Object.keys(collections) as CollectionName[]` — the parameter is a `Record<CollectionName, …>`, so every key IS a CollectionName by construction; TypeScript types `Object.keys` as `string[]` and cannot carry that. Same guard and same reason as `publish-preview.ts` below. MEASURED: removing one collection from the call site fails `tsc` with TS2741 naming the missing key.",
