...
import { overlayCollection } from "../../lib/studio/draft-overlay.ts";
...
{
  const r = overlayCollection(live, {}, []);
...
{
  const r = overlayCollection(live, { b: { slug: "b", orderIndex: 2, v: "DRAFT-B" } }, []);
...
// (8) TIE orderIndex -> shared orderIndex preserves LIVE order (stable sort + live-order Map)
{
  ...
  const r = overlayCollection(tied, {}, []);
  ...
  const r2 = overlayCollection(tied, { x: { slug: "x", orderIndex: 5, v: "DRAFT-X" } }, []);
